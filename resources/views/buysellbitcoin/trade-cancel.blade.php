    ...
    <div class="container-fluid online_sell_buyer_fluid">
        <div class="container">
            ...
            <div class="col-md-12">
                <div class="well">
                    Trade status:
                    <strong id="trade_status">Trade canceled by buyer.</strong>
                </div>
            </div>
                ...
                <div class="online_sell_buyer_L">
                    <div class="escrowbox well">
                        <div id="payment_instructions" style="">
                           <p>This trade was closed at May 28, 2018, 10:07 a.m.. You can still send messages to the trade partner.</p>
                            <br>
                    ...
                    <div class="Verify_row_1">
                        <div class="Verify_div">
                            <h3 class="Verify_h3">Leave feedback for excoins.me</h3>
                        </div>
                        <div class="Verify_row_1">
                                <div class="Verify_div">
                                    <div class="Verify_redio_div">
                                        <form method="POST" action="{{ route('leave.feedback',['name'=>$trade->offer->author->name]) }}">
                                            {{ csrf_field() }}
                                            <input type="hidden" name="user_id" value="{{ Auth::user()->id }}">
                                            <input type="hidden" name="feedback_user_id" value="{{ $trade->offer->author->id }}">
                                            <input type="hidden" name="offer_id" value="{{ $trade->offer->id }}">

                                            <input class="Verify_input" type="radio" name="ratting" value="positive"> <span class="Verify_span"> Positive</span>
                                            <p class="Verify_p Verify_p2">Give your trading partner positive feedback to incease his reputation.</p> 
        
                                            <input class="Verify_input" type="radio" name="ratting" value="neutral"> <span class="Verify_span2"> Neutral</span>
                                            <p class="Verify_p Verify_p2">Give your trading partner neutral feedback that does not affect his reputation.</p> 
        
                                            <input class="Verify_input" type="radio" name="ratting" value="negative"> <span class="Verify_span2"> Distrust</span>
                                            <p class="Verify_p Verify_p2">Give your trading partner negative feedback that decreases his reputation and block his account, this prevents him from trading with you again.</p> 
        
                                            <textarea class="send_row_message" placeholder="Write your message" name="message"></textarea>
                                            <button type="submit" class="btn btn-outline-secondary send_row_button1">Update feedback</button>
                                        </form>
                                    </div>
...
<script src="{{ asset('chat/js/talk.js') }}"></script>
   ...
   <script type="text/javascript">
        // Payment complete countdown
        $(function() {
            if (Cookies.get('extime')) {
                var count = Cookies.get('extime');
            }


        function timer() {
            count = count-5;
            Cookies.set('extime', count);
            if (Cookies.get('extime') <= 0) {
            // document.getElementById("paymentTimerDiv").innerHTML = "<p><b>Payment window closed. Please do not advance with the purchase.</b></p>";
            // clearInterval(counter);
            } else if (Cookies.get('extime') <= 60) {
            $("#timer1").text("<1")
            } else {
            $("#timer1").text((Cookies.get('extime') / 60).toFixed());
            }
        }

        if(Cookies.get('extime')) {
            // Decrease paymentcomplete countdown every minute
